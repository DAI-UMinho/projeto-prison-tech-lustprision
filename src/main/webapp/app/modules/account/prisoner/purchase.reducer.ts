...
export const ACTION_TYPES = {
  FETCH_PURCHASE_PRODUCT_INFO_LIST: 'purchase/FETCH_PURCHASE_PRODUCT_INFO_LIST',
...
export default (state: PurchaseState = initialState, action): PurchaseState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.FETCH_PURCHASE_PRODUCT_INFO_LIST):
    ...
    case FAILURE(ACTION_TYPES.FETCH_PURCHASE_PRODUCT_INFO_LIST):
    ...
    case SUCCESS(ACTION_TYPES.FETCH_PURCHASE_PRODUCT_INFO_LIST):
      return {
        ...state,
        loading: false,
        infoProducts: action.payload.data
      };
...
export const getPurchaseProductInfoList: ICrudGetAllAction<any> = id => {
  const requestUrl = `${apiUrl}/${id}/pres-products`;
  return {
    type: ACTION_TYPES.FETCH_PURCHASE_PRODUCT_INFO_LIST,
