...
describe('Product e2e test', () => {
  ...
  it('should load create Product page', async () => {
    ...
    expect(await productUpdatePage.getPageTitle().getAttribute('id')).to.match(/lustPrisionApp.product.home.createOrEditLabel/);
  ...
  it('should create and save Products', async () => {
    ...
    expect(await productUpdatePage.getNameProdInput()).to.match(/nameProd/);
    await productUpdatePage.setPriceInput('5');
    expect(await productUpdatePage.getPriceInput()).to.eq('5');
    await productUpdatePage.setSelerInput('seler');
    expect(await productUpdatePage.getSelerInput()).to.match(/seler/);
    await productUpdatePage.setDescriptionProdInput('descriptionProd');
    expect(await productUpdatePage.getDescriptionProdInput()).to.match(/descriptionProd/);
    await productUpdatePage.setQuantyInStockInput('5');
  ...
  it('should delete last Product', async () => {
    ...
    expect(await productDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/lustPrisionApp.product.delete.question/);
